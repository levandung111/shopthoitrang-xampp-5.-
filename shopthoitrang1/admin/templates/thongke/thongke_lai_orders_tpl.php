...
<div class="control_frm">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
	            <li><a href="index.php?com=thongke&act=man"><span>Thống Kê tiền lãi đơn hàng theo ngày</span></a></li>
...
<div class="widget">
  <div class="titlee" style="padding-bottom:5px;">
    ...
    <form name="search" action="index.php" method="GET" class="form giohang_ser">
      ...
      <input name="act" value="thongke_lai_orders" type="hidden" />
      ...
      <input class="form_or" name="ngaybd" data-format="dd/mm/yyyy" id="datefm" type="text" value="<?=$_GET['ngaybd']?>" placeholder="Từ ngày.."/>
      <input class="form_or" name="ngaykt" data-format="dd/mm/yyyy" id="dateto" type="text" value="<?=$_GET['ngaykt']?>" placeholder="Đến ngày.." />
     ...
     // print_r($result_ctdonhang);
		...
		for($i=0,$count_donhang=count($items);$i<$count_donhang;$i++){	
			...
			$tongtiennhap+=	$items[$i]['tonggianhap'];
				
		}
				
		?>
    
    <b>Số đơn hàng :</b> &nbsp;<span class="count_info_bg"><?=count($items)?></span> 
	<br>	<b>Tổng giá bán được tất cả đơn hàng:</b>&nbsp;<span><?=number_format($tongtien,0,',','.')?>&nbsp;VNĐ</span>
	
	<br> <b>Tổng giá nhập tất cả đơn hàng:</b>&nbsp;<span><?=number_format($tongtiennhap,0,',','.')?>&nbsp;VNĐ</span>
	
	<br> <b>Tổng tiền lãi tất cả đơn hàng:</b>&nbsp;<span><?=number_format(($tongtien-$tongtiennhap),0,',','.')?>&nbsp;VNĐ</span>
  ...
  <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" >
    <thead>
      <tr>
       ...
       <td class="sortCol" width="120"><div>Mã đơn hàng<span></span></div></td>     
        ...
        <td width="150">Số tiền giá bán</td>
		<td width="150">Số tiền giá nhập</td>
		<td width="150">Số tiền lãi</td>
    ...
    <tbody>
       ...
       <td align="center">
		...
		<td align="center">
           <?=number_format($items[$i]['tonggianhap'],0, ',', '.')?>&nbsp;VNĐ
        </td>
		
		
		<td align="center">
           <?=number_format( ($items[$i]['tonggia']-$items[$i]['tonggianhap']),0, ',', '.')?>&nbsp;VNĐ
        </td>
...
function onSearch(evt) {	
		...
		location.href = "index.php?com=thongke&act=thongke_lai_orders&datefm="+datefm+"&dateto="+dateto+"&status="+status;
