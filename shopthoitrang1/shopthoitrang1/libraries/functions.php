...
function get_totalprice_gianhap($id=0){
		global $d;
		if($id>0){
			$d->reset();
			$sql="select gia,soluong,id_product,id,id_order from #_order_detail where id_order='".$id."'";
			$d->query($sql);
			...
			for($i=0,$count=count($result);$i<$count;$i++) { 
							...
							$d->reset();
							$sql="select id,gia,gianhap from #_product where id='".$result[$i]["id_product"]."'";
							$d->query($sql);
							$rs_kq=$d->fetch_array();
			
			
				$tongtien+=	$rs_kq['gianhap']*$result[$i]['soluong'];	
			}
			return $tongtien;
		}else return 0;
	}
...
function get_numberorder_pro($id=0){
  global $d, $row;
  $sql="select  COUNT(*) as num from #_order_detail where id_product=".$id."  ";
  $d->query($sql);
  $row=$d->fetch_array();
	return $row['num'];
...
//hàm chọn số ngẫu nhiên..cái này hay hihi
