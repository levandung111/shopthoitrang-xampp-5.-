...
function save_item() {
    ...
    if (empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=product&act=man&id_list=$_GET[id_list]&typechild=$_GET[typechild]");
    ...
    if ($id) {
        ...
        $data['gianhap'] = $_POST['gianhap']; //Tổng giá
...
function delete_item() {
    ...
    if (isset($_GET['id'])) {
        $id = themdau($_GET['id']);
        $d->reset();
        $sql = "select id from table_store where id='" . $id . "'";
        $d->query($sql);
        if ($d->num_rows() > 0) {
            $sql = "delete from table_store where id='" . $id . "'";
            $d->query($sql);
        }
        if ($d->query($sql)) {
            $_SESSION['msg_delete_store'] = 'Xóa dữ liệu thành công';
            redirect($url_back);
        } else {
            $_SESSION['msg_delete_store'] = 'Xóa dữ liệu bị lỗi';
            transfer("Xóa dữ liệu bị lỗi", $url_back);
        }
    } else transfer("Không nhận được dữ liệu", $url_back);
}
...
function save_sub() {
    ...
    $file_name = changeTitle($_POST['ten_vi']) . fns_Rand_digit(0, 3, 5);
    if (empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_sub&typeparent=$_GET[typeparent]");
    $id = isset($_POST['id']) ? themdau($_POST['id']) : "";
    if ($id) {
...
function save_cat() {
    ...
    if (empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_cat&typeparent=$_GET[typeparent]");
    $id = isset($_POST['id']) ? themdau($_POST['id']) : "";
...
function save_loai() {
    ...
    $file_name = changeTitle($_POST['ten_vi']) . fns_Rand_digit(0, 3, 5);
    if (empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_item&typeparent=$_GET[typeparent]");
    $id = isset($_POST['id']) ? themdau($_POST['id']) : "";
    if ($id) {
...
function save_list() {
    ...
    if (empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_list&typeparent=$_GET[typeparent]");
...
function insert_tag($itemid = '0', $tag = '') {
    ...
    for ($i = 0; $i < count($tagarray); $i++) {
        ...
        if ($usetag == "") continue;
        $query = "INSERT INTO table_tag (item_id,tag) VALUES ($itemid,'$usetag')";
        mysql_query($query);
    }
}
...
function shows_tag($itemid = '0') {
    global $d;
    $query = "select tag from #_tag where item_id='$itemid'";
    $d->query($query);
...
function delete_tag($itemid = '0') {
    global $d;
    $sql = "delete from #_tag where item_id='" . $itemid . "'";
    $d->query($sql);
}
