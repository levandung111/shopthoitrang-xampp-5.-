...
function load_product(id_item,type){
	$.ajax({
		...
		success: function(res){
			$('#litsProduct').html(res);
		}
	});
}

function save_bill(data_insert,type){
	$.ajax({
		url:'ajax/products.php',
		type: "POST",
		dataType: "html",
		data: {data_insert:data_insert,type:type},
		success: function(res){
			alert('Tạo hoá đơn nhập hàng thành công!');
			location.href = 'index.php?com=store&act=list&typechild=product'
		}
	});
}
