    ...
    <div class="control_frm" style="margin-top:0;">
        <div style="float:left;">
            <input type="button" class="blueB" value="Thêm"
                   onclick="location.href='index.php?com=product&act=add&typechild=<?= @$_GET['typechild'] ?>'"/>
            <input type="button" class="blueB" value="Hiện"
                   onclick="ChangeAction('index.php?com=product&act=man&typechild=<?= @$_GET['typechild'] ?>&multi=show');return false;"/>
            <input type="button" class="blueB" value="Ẩn"
                   onclick="ChangeAction('index.php?com=product&act=man&typechild=<?= @$_GET['typechild'] ?>&multi=hide');return false;"/>
            <input type="button" class="blueB" value="Xoá" id="xoahet"
                   onclick="ChangeAction('index.php?com=product&act=man&typechild=<?= @$_GET['typechild'] ?>&multi=del');return false;"/>


        </div>
        <div style="float:right;">
            <div class="selector">
                Tìm kiếm: <input name="keyword" id="keyword" type="text" value="<?= @$_REQUEST['keyword'] ?>"/> <input
                    type="button" value=" Tìm " onclick="onSearch(event)"/>
            </div>
        </div>
    ...
    <div class="widget">
        ...
        <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" >
            ...
            <tr>
                ...
                <td class="tb_data_small"><a href="#" class="tipS" style="margin: 5px;">Mã sản phẩm</a></td>

                <?php if ($danhmuc_cap1 == "on") { ?>

                    <td class="tb_data_small">
                        <div><?= get_main_list(); ?><span></span></div>
                    </td>
                ...
                <?php if ($danhmuc_cap2 == "on") { ?>
                    <td class="tb_data_small">
                        <div><?= get_main_cat(); ?><span></span></div>
                    </td>
                ...
                <?php if ($danhmuc_cap3 == "on") { ?>
                    <td class="tb_data_small">
                        <div><?= get_main_item(); ?><span></span></div>
                    </td>
                <?php } ?>

                <?php if ($danhmuc_cap4 == "on") { ?>
                    <td class="tb_data_small">
                        <div><?= get_main_sub(); ?><span></span></div>
                    </td>
                ...
                <?php if ($check_noibat == "on") { ?>
                    <td class="tb_data_small">SanPhamNoiBat</td>
                <?php } ?>
            ...
            <tr>
                <td colspan="10">
                    <div
                        class="pagination">  <?= pagesListLimitadmin($url_link, $totalRows, $pageSize, $offset) ?>     </div>
                </td>
            </tr>
            </tfoot>
            ...
            <?php for ($i = 0, $count = count($items); $i < $count; $i++) { ?>
             ...
             <?= $i +1 ?></td>
                    ...
                    <td class="title_name_data">
                        <a href="index.php?com=product&act=edit&typechild=<?= @$_GET['typechild'] ?>&id=<?= $items[$i]['id'] ?>&id_list=<?= $items[$i]['id_list'] ?>&id_cat=<?= $items[$i]['id_cat'] ?>&id_item=<?= $items[$i]['id_item'] ?>&id_sub=<?= $items[$i]['id_sub'] ?>"
                    ...
                    <?php if ($check_moi == "on") { ?>
                        ...
                        <td align="center">
                            ...
                            if ($items[$i]['spmoi'] > 0) {
                                ?>
                                <a href="index.php?com=product&act=man&spmoi=<?= $items[$i]['id'] ?>&typechild=<?= @$_GET['typechild'] ?>"
                                   title="" class="smallButton tipS" original-title="Click để ẩn"><img
                                        src="./images/icons/color/tick.png" alt=""></a>
                                <?php
                            } else {
                                ?>
                                <a href="index.php?com=product&act=man&spmoi=<?= $items[$i]['id'] ?>&typechild=<?= @$_GET['typechild'] ?>"
                                   title="" class="smallButton tipS" original-title="Click để hiện"><img
                                        src="./images/icons/color/xicon.png" alt=""></a>
                            <?php }
                            ?>
                    ...
                    <?php if ($check_noibat == "on") { ?>
                        ...
                        <td align="center">
                            ...
                            if ($items[$i]['spnoibat'] > 0) {
                                ?>
                                <a href="index.php?com=product&act=man&spnoibat=<?= $items[$i]['id'] ?>&typechild=<?= @$_GET['typechild'] ?>"
                                   title="" class="smallButton tipS" original-title="Click để ẩn"><img
                                        src="./images/icons/color/tick.png" alt=""></a>
                                <?php
                            } else {
                    ...
                    <td><?= $items[$i]['soluong'] ?></td>

                    <td align="center">
                        <?php
                        if (@$items[$i]['hienthi'] == 1) {
                            ?>
                            <a href="index.php?com=product&act=man&typechild=<?= @$_GET['typechild'] ?>&hienthi=<?= $items[$i]['id'] ?>"
                               title="" class="smallButton tipS" original-title="Click để ẩn"><img
                                    src="./images/icons/color/tick.png" alt=""></a>
                        <?php } else { ?>
                            <a href="index.php?com=product&act=man&typechild=<?= @$_GET['typechild'] ?>&hienthi=<?= $items[$i]['id'] ?>"
                    ...
                    <td class="actBtns">
                        <a href="index.php?com=product&act=edit&typechild=<?= @$_GET['typechild'] ?>&id=<?= $items[$i]['id'] ?>&id_list=<?= $items[$i]['id_list'] ?>&id_cat=<?= $items[$i]['id_cat'] ?>&id_item=<?= $items[$i]['id_item'] ?>&id_sub=<?= $items[$i]['id_sub'] ?>"
                           title="" class="smallButton tipS" original-title="Sửa sản phẩm"><img
                                src="./images/icons/dark/1234.png" alt=""></a>
                        <a href="index.php?com=product&act=delete&typechild=<?= @$_GET['typechild'] ?>&id=<?= $items[$i]['id'] ?>&id_list=<?= $items[$i]['id_list'] ?>&id_cat=<?= $items[$i]['id_cat'] ?>"
                           onclick="CheckDelete('index.php?com=product&act=delete&typechild=<?= @$_GET['typechild'] ?>&id=<?= $items[$i]['id'] ?>&id_list=<?= $items[$i]['id_list'] ?>&id_cat=<?= $items[$i]['id_cat'] ?>&id_item=<?= $items[$i]['id_item'] ?>&id_sub=<?= $items[$i]['id_sub'] ?>'); return false;"
                           title="" class="smallButton tipS" original-title="Xóa sản phẩm"><img
